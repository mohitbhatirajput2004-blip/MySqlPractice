...
select * , lead(salary , 2 ) over() from employees; -- select the +2 salary of current one

select * , lag(salary ) over() from employees; -- select the last salary  in  the table


select * , salary - lag(salary ) over(partition by department order by hire_date) from employees; -- same for lag
...
select * from
(select  *, row_number() over(partition by department order by salary ) as row_value from employees) temp_data  where row_value <= 2;
 

select * , salary - avg(salary)  over(partition by department ) as avgVSsalary from employees; 
